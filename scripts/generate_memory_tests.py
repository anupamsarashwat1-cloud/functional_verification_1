...
memory_dir = "/home/anupam-sarashwat/project_penguin_verification/memory"
cocotb_dir = "/home/anupam-sarashwat/project_penguin_verification/verification/cocotb/memory"

os.makedirs(cocotb_dir, exist_ok=True)

v_files = [f for f in os.listdir(memory_dir) if f.endswith('.v')]
...
for vf in v_files:
    ...
    with open(os.path.join(memory_dir, vf), 'r') as f:
        content = f.read()
        
    inputs = re.findall(r'input\s+wire\s+(?:\[.*?\]\s*)?(\w+)', content)
    
    clk_name = "clk"
    if "ddr_clk" in inputs:
        clk_name = "ddr_clk"
    elif "mem_clk" in inputs:
        clk_name = "mem_clk"
...
COMPILE_ARGS += -I../../../../common -I../../../../memory -I../../../../interconnect
...
print(f"Memory test environments generated for {len(v_files)} modules.")
