...
storage_dir = "/home/anupam-sarashwat/project_penguin_verification/storage"
cocotb_dir = "/home/anupam-sarashwat/project_penguin_verification/verification/cocotb/storage"

os.makedirs(cocotb_dir, exist_ok=True)

v_files = [f for f in os.listdir(storage_dir) if f.endswith('.v')]
...
for vf in v_files:
    ...
    with open(os.path.join(storage_dir, vf), 'r') as f:
        content = f.read()
    ...
    if "sys_clk" in inputs:
        clk_name = "sys_clk"
    elif "usb_clk" in inputs:
        clk_name = "usb_clk"
    elif "mmc_clk" in inputs:
        clk_name = "mmc_clk"
...
COMPILE_ARGS += -I../../../../common -I../../../../storage
...
print(f"Storage test environments generated for {len(v_files)} modules.")
